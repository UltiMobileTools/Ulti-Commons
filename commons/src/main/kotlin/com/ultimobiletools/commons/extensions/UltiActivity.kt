...
import com.ultimobiletools.commons.views.CustomTextView
import java.io.*
import java.util.*
...
fun Activity.launchPurchaseThankYouIntent() {
    hideKeyboard()
    try {
        launchViewIntent("market://details?id=com.ultimobiletools.thankyou")
...
fun Activity.openPathIntent(
    ...
    ensureBackgroundThread {
        ...
        Intent().apply {
            ...
            if (applicationId == "com.ultimobiletools.gallery.pro" || applicationId == "com.ultimobiletools.gallery.pro.debug") {
...
fun BaseUltiActivity.launchCallIntent(recipient: String, handle: PhoneAccountHandle? = null) {
    handlePermission(PERMISSION_CALL_PHONE) {
        ...
        Intent(action).apply {
            ...
            if (isDefaultDialer()) {
                val packageName = if (baseConfig.appId.contains(".debug", true)) "com.ultimobiletools.dialer.debug" else "com.ultimobiletools.dialer"
                val className = "com.ultimobiletools.dialer.activities.DialerActivity"
